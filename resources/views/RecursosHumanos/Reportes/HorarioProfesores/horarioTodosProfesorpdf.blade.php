...
<head>
    ...
    <title>TODOS LOS HORARIOS</title>
...
<footer class="footerConstancia">
    ...
    <img src="{{ asset('logos/reportes/LogoUpv2023.png') }}" style="margin-left: 195px; margin-top: -15px; width: 160px;"  alt="...">
    <img src="{{ asset('logos/reportes/LOGOCGUTYP.png') }}" style="margin-left: 5px; margin-top: -2px; width: 130px;" alt="...">
</footer>
...
<main>
    ...
    @php
        $profesorSize = count($profesor);
        $i = 0;
    @endphp

    @foreach($profesor as $docente)
...
<table class="table-nombreProfesor">
	<thead>
		<tr class="font-weight-bold">
			<th style="text-align: center;">{{ $docente->numero}}</th>
			<th style="text-align: left;">{{$docente->Empleado}}</th>
        </tr>
	</thead>
...
<table class="table-horarioProfesor" style="margin-top: 10px;">
	<thead>
		<tr class="font-weight-bold">
            ...
            @php
                ...
                foreach($horarioMaterias as $h){
                    if($h->idEmpleado == $docente->idEmpleado){
                        if($h->numDia == 5){
                            $existeSabado = 1;
                        }
                }
 ...
 <!-- Aquí hace la validación si existe o no el sabado -->
	...
	<tbody>
        ...
        @if($horas->idEmpleado == $docente->idEmpleado)
            <tr>
                ...
                <td style="text-align: left; width: 85px;">
                    @foreach ($horarioMaterias as $matLunes)
                        @if($matLunes->idEmpleado == $docente->idEmpleado)
                ...
                <td style="text-align: left; width: 85px;">
                    @foreach ($horarioMaterias as $matMartes)
                        @if($matMartes->idEmpleado == $docente->idEmpleado)
                ...
                <td style="text-align: left; width: 85px;">
                    @foreach ($horarioMaterias as $matMiercoles)
                        @if($matMiercoles->idEmpleado == $docente->idEmpleado)
                ...
                <td style="text-align: left; width: 85px;">
                    @foreach ($horarioMaterias as $matJueves)
                        @if($matJueves->idEmpleado == $docente->idEmpleado)
                ...
                <td style="text-align: left; width: 85px;">
                    @foreach ($horarioMaterias as $matViernes)
                        @if($matViernes->idEmpleado == $docente->idEmpleado)
                ...
                <td style="text-align: left; width: 85px;">
                    @foreach ($horarioMaterias as $matSabado)
                        @if($matSabado->idEmpleado == $docente->idEmpleado)
